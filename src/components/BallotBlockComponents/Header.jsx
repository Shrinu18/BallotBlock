"use client";
import React, { useState } from "react";
import UserDetails from "../Modal/UserDetails";
import Polling from "../Modal/Polling";

export default function Header({ uData }) {
  const [showUserDetails, setShowUserDetails] = useState(false);
  const [showPolling, setShowPolling] = useState(false);
  return (
    <div className="">
      <div className="flex items-center min-h-[5vh]">
        <div className="md:container md:mx-auto w-full">
          <div className="flex items-center justify-between py-3 mx-2 md:mx-0">
            <button className="cursor-pointer">
              <h1 className="text-[#FFFFFF] md:text-2xl text-xl font-bold">
                Ballot<span className="font-light">Block</span>
              </h1>
            </button>
            <div className="flex items-center md:gap-4 gap-2">
              <div className="hidden sm:block">
                <button
                  type="button"
                  onClick={() => setShowPolling(true)}
                  className="flex items-center gap-1 bg-[#FFFFFF] text-[#000000] font-bold px-4 py-2 rounded-xl hover:bg-[#007FFF] cursor-pointer hover:rounded-4xl transition-all duration-300"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    className="w-6"
                    fill={"none"}
                  >
                    <path
                      d="M14.236 5.29178C14.236 4.77191 14.236 4.51198 14.1789 4.29871C14.0238 3.71997 13.5717 3.26793 12.9931 3.11285C12.4315 2.96238 11.5684 2.96238 11.0068 3.11285C10.4281 3.26793 9.97609 3.71997 9.82101 4.29871C9.76387 4.51198 9.76387 4.77191 9.76387 5.29178C9.76387 6.34588 9.76387 9.109 9.43641 9.43647C9.10894 9.76393 6.34582 9.76393 5.29172 9.76393C4.77185 9.76393 4.51192 9.76393 4.29865 9.82107C3.71991 9.97615 3.26787 10.4282 3.11279 11.0069C2.96232 11.5685 2.96232 12.4315 3.11279 12.9931C3.26787 13.5718 3.71991 14.0239 4.29865 14.1789C4.51192 14.2361 4.77185 14.2361 5.29172 14.2361C6.34582 14.2361 9.10894 14.2361 9.43641 14.5635C9.76387 14.891 9.76387 15.418 9.76387 16.4721C9.76387 16.992 9.76387 19.4881 9.82101 19.7013C9.97609 20.28 10.4281 20.7321 11.0068 20.8871C11.5684 21.0376 12.4315 21.0376 12.9931 20.8871C13.5717 20.7321 14.0238 20.28 14.1789 19.7013C14.236 19.4881 14.236 16.992 14.236 16.4721C14.236 15.418 14.236 14.891 14.5635 14.5635C14.8909 14.2361 17.654 14.2361 18.7082 14.2361C19.228 14.2361 19.488 14.2361 19.7013 14.1789C20.28 14.0239 20.732 13.5718 20.8871 12.9931C21.0376 12.4315 21.0376 11.5685 20.8871 11.0069C20.732 10.4282 20.28 9.97615 19.7013 9.82107C19.488 9.76393 19.228 9.76393 18.7082 9.76393C17.654 9.76393 14.8909 9.76393 14.5635 9.43647C14.236 9.109 14.236 6.34588 14.236 5.29178Z"
                      stroke="currentColor"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                  </svg>
                  Polling
                </button>
              </div>
              <div className="sm:hidden">
                <button
                  type="button"
                  onClick={() => setShowPolling(true)}
                  className="flex items-center gap-1 bg-[#FFFFFF] text-[#000000] font-bold p-1 rounded-xl hover:bg-[#007FFF] cursor-pointer hover:rounded-4xl transition-all duration-300"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    className="w-6"
                    fill={"none"}
                  >
                    <path
                      d="M14.236 5.29178C14.236 4.77191 14.236 4.51198 14.1789 4.29871C14.0238 3.71997 13.5717 3.26793 12.9931 3.11285C12.4315 2.96238 11.5684 2.96238 11.0068 3.11285C10.4281 3.26793 9.97609 3.71997 9.82101 4.29871C9.76387 4.51198 9.76387 4.77191 9.76387 5.29178C9.76387 6.34588 9.76387 9.109 9.43641 9.43647C9.10894 9.76393 6.34582 9.76393 5.29172 9.76393C4.77185 9.76393 4.51192 9.76393 4.29865 9.82107C3.71991 9.97615 3.26787 10.4282 3.11279 11.0069C2.96232 11.5685 2.96232 12.4315 3.11279 12.9931C3.26787 13.5718 3.71991 14.0239 4.29865 14.1789C4.51192 14.2361 4.77185 14.2361 5.29172 14.2361C6.34582 14.2361 9.10894 14.2361 9.43641 14.5635C9.76387 14.891 9.76387 15.418 9.76387 16.4721C9.76387 16.992 9.76387 19.4881 9.82101 19.7013C9.97609 20.28 10.4281 20.7321 11.0068 20.8871C11.5684 21.0376 12.4315 21.0376 12.9931 20.8871C13.5717 20.7321 14.0238 20.28 14.1789 19.7013C14.236 19.4881 14.236 16.992 14.236 16.4721C14.236 15.418 14.236 14.891 14.5635 14.5635C14.8909 14.2361 17.654 14.2361 18.7082 14.2361C19.228 14.2361 19.488 14.2361 19.7013 14.1789C20.28 14.0239 20.732 13.5718 20.8871 12.9931C21.0376 12.4315 21.0376 11.5685 20.8871 11.0069C20.732 10.4282 20.28 9.97615 19.7013 9.82107C19.488 9.76393 19.228 9.76393 18.7082 9.76393C17.654 9.76393 14.8909 9.76393 14.5635 9.43647C14.236 9.109 14.236 6.34588 14.236 5.29178Z"
                      stroke="currentColor"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                  </svg>
                </button>
              </div>
              <div className="hidden sm:block">
                <button
                  type="button"
                  onClick={() => setShowUserDetails(true)}
                  className="group cursor-pointer bg-[#FFFFFF] rounded-full md:p-1 p-0.5 flex md:gap-2 gap-1 items-center"
                >
                  <span className="md:size-10 size-6 flex items-center justify-center bg-[#007FFF] md:text-lg text-sm rounded-full font-bold group-hover:bg-[#000000] transition-all duration-200 text-[#FFFFFF]">
                    {`${uData ? uData.username : "user"}`
                      .substring(0, 2)
                      .toUpperCase()}
                  </span>
                  <span className="text-[#000000] font-bold md:pr-2 pr-1 md:text-base text-sm">
                    {uData ? uData.balance : "0"}{" "}
                    <span className="text-[#007FFF]">BaBl</span>
                  </span>
                </button>
              </div>
              <div className="sm:hidden">
                <button
                  type="button"
                  onClick={() => setShowUserDetails(true)}
                  className="group cursor-pointer bg-[#FFFFFF] rounded-full md:p-1 p-0.5 flex md:gap-2 gap-1 items-center"
                >
                  <span className="md:size-10 size-7 flex items-center justify-center bg-[#007FFF] md:text-lg text-sm rounded-full font-bold group-hover:bg-[#000000] transition-all duration-200 text-[#FFFFFF]">
                    {`${uData ? uData.username : "user"}`
                      .substring(0, 2)
                      .toUpperCase()}
                  </span>
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
      {showUserDetails && (
        <UserDetails
          onClose={() => setShowUserDetails(false)}
          show={showUserDetails}
          uData={uData}
        />
      )}
      {showPolling && (
        <Polling
          onClose={() => setShowPolling(false)}
          show={showPolling}
          uData={uData}
        />
      )}
    </div>
  );
}
