"use client";
import { pollCreation, voteCreation } from "@/services/allService";
...
export default function CreateVote({
  ...
  const handleCreateVote = async (e) => {
    ...
    try {
      ...
      const updatedPoll = {
        ...pData,
        nonce: selectedOptionDesc,
        selectedOption,
        selectedOptionDesc,
      };

      const { success, error } = await voteCreation(walletaddress, updatedPoll);
    ...
    <div className="overflow-x-clip">
      <div className="fixed inset-0 z-40 bg-[#0080FF75] flex items-center justify-center">
        <div className="md:w-[600px] mx-2 w-full md:p-8 p-4 bg-[#000000] md:rounded-4xl rounded-2xl">
          ...
          <p className="text-[#FFFFFF] md:text-lg text-sm font-medium">
            <span className="text-[#0080FF] font-bold">2 BaBl Coins</span> is
            Required.
          </p>
          <p className="text-[#FFFFFF] md:text-lg text-sm font-medium">
            After Payment Balance :{" "}
            <span className="text-[#0080FF] font-bold">
              {uData ? uData.balance - 2 : "0"} BaBl Coins
            </span>
          </p>
